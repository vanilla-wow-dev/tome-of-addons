...
| **Titel (roh)** | `## Title` einschließlich Farbcodes. Sortierschlüssel des Clients, für die Anzeige ungeeignet. |
| **Titel** | `## Title` aus der `.toc` — reine Anzeige, nicht eindeutig. Zwei Addons dürfen denselben Titel tragen; unterschieden wird über `addon_id` und Tree-Hash. |
...
| E-45 | Die Addon-Liste sortiert nach dem **rohen** `## Title` inklusive Farbcodes, kleingeschrieben und zeichenweise | So sortiert der Client. Belegt, nicht vermutet: `Interface\GlueXML\AddonList.lua` (aus `Data/patch.MPQ` extrahiert) sortiert überhaupt nicht, sondern läuft über `GetAddOnInfo(1..n)` — die Reihenfolge entsteht im C-Teil. Die Regel wurde aus Screenshots des AddOn-Fensters rekonstruiert und reproduziert Anfang, Übergang und Ende der Liste Zeile für Zeile; die TypeScript-Umsetzung stimmt über alle 258 Addons mit der verifizierten Referenz überein. Drei ASCII-Eigenheiten erklären sie: `[` (0x5B) < `a` steht vorn, `\|` (0x7C) > `z` schiebt alle gefärbten Titel ans Ende, und innerhalb der gefärbten entscheidet der Hex-Wert. Kein `localeCompare` — dessen Kollation würde genau diese Eigenheiten wegbügeln; aus demselben Grund entfällt die natürliche Zahlensortierung. |
| E-42 | Bei doppelten `.toc`-Direktiven gewinnt die **letzte** Angabe (Korrektur zu E-12) | Ein zeilenweise lesender Parser weist beim Lesen zu — das ist auch, was der Autor einer solchen Datei meint. Im Bestand genau einmal aufgetreten: `Config.toc` trug in der Zeile über dem gemeinten Titel einen Copy-Paste-Rest aus dem Nachbar-Addon `Addons`, wodurch zwei verschiedene Addons unter demselben Namen erschienen. |
| E-43 | Titel werden aus **einem** Parser in Rust in farbige Abschnitte zerlegt; das Frontend rendert nur | Ein zweiter Escape-Parser in TypeScript wäre genau die Divergenz aus E-8. `strip_ui_escapes` ist aus denselben Abschnitten abgeleitet, ein Auseinanderlaufen also ausgeschlossen. Gerendert wird über `<span>`-Elemente, nie über `v-html` — der Inhalt stammt aus fremden Addon-Dateien. |
| E-44 | Autorenfarben werden auf WCAG-AA nachgezogen, statt sie roh zu übernehmen oder zu verwerfen | Addon-Autoren färben für den dunklen Spiel-Hintergrund; `ffffff` wäre auf Pergament unsichtbar. Abgedunkelt wird durch Skalieren im linearen Licht (Farbton bleibt), aufgehellt durch Mischen mit Weiß (Skalieren würde Kanäle sättigen und den Farbton verziehen). Wer den Kontrast schon erreicht, bleibt unangetastet. |
...
| E-39 | Veraltete Addons sind **vorgabegemäß ausgeblendet**, wenn der Client sie nicht lädt | Die Liste soll spiegeln, was im Spiel ankommt. Der Bezug ist die CVar `checkAddonVersion` aus `WTF/Config.wtf`: `0` heißt „lädt veraltete", alles andere heißt „prüft Versionen". Fehlt sie, gilt der Client-Vorgabewert — WoW 1.12 schreibt CVars erst beim Ändern, unverändert ist die Prüfung an. Das Häkchen „Auch veraltete zeigen" hebt die Ausblendung auf. |
| E-40 | Übersprungene Ordner erscheinen nur in der Addon-Ansicht | Welcher Ordner kein gültiges `.toc` hat, ist eine Eigenschaft der Installation, nicht eines Charakters. |
| E-41 | Scrollbereiche reservieren den Platz für den Balken über eigenes `padding-right`, nicht über `scrollbar-gutter` allein | WebKitGTK zeichnet Overlay-Balken, die keinen Platz beanspruchen und sich damit über die letzte Spalte legen. Das Polster wirkt unabhängig von der Engine. |
